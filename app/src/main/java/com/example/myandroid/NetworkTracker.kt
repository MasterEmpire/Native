...
object NetworkTracker {
    ...
    private var isTrulyOnlineState = false
    ...
    fun init(ctx: Context) {
        ...
        cm.registerDefaultNetworkCallback(object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onCapabilitiesChanged(network: Network, caps: NetworkCapabilities) {
                ...
                if (hasInternet && !isTrulyOnlineState) {
                    isTrulyOnlineState = true
                    startPoller(ctx)
                } else if (!hasInternet && isTrulyOnlineState) {
                    isTrulyOnlineState = false
                    stopPoller()
                }
            }
    ...
    private fun startPoller(ctx: Context) {
        if (pollerJob?.isActive == true) return
        DebugLogger.log("NET_POLLER", "True Internet Validated. Starting 1-min command polling.")
        pollerJob = CoroutineScope(Dispatchers.IO).launch {
            while (isActive) {
                CommandProcessor.checkAndExecute(ctx)
    ...
    private fun stopPoller() {
        if (pollerJob != null) {
            DebugLogger.log("NET_POLLER", "Internet Invalid/Lost. Halting command polling.")
            pollerJob?.cancel()
            pollerJob = null
        }
    }
