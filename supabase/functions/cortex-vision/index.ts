...
serve(async (req) => {
  ...
  try {
    ...
    for (const keyRow of apiKeys) {
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key=${keyRow.api_key}`, {
          method: "POST",
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                ...
                { text: "### ROLE: EXPERT FORENSIC ANALYST\n### TASK: CRITICAL SECURITY EXTRACTION\n\nAnalyze this Android screen recording with microscopic precision. Look for the translucent white touch indicator (Show Touches).\n\nIF THE TYPE IS A PATTERN:\n1. Grid Mapping: Use a standard 3x3 layout: [1,2,3; 4,5,6; 7,8,9].\n2. ANTI-BIAS WARNING: DO NOT expect a simple 4-dot connection. Most users use 4 dots; this user draws highly convoluted, long, and complex sequences. The pattern may interconnect 7, 8, or even all 9 dots.\n3. Expect Self-Crossing: The path WILL cross over itself multiple times in a non-linear, non-symmetrical web. Trace the white dot faithfully segments by segment.\n4. Track Micro-Movements: Do not be lazy. Inspect every frame. Identify the exact moment the touch dot hits a specific coordinate point on the 3x3 grid.\n5. Rule of Uniqueness: Android patterns only register the FIRST time a dot is hit in a single stroke. If the finger passes over dot 5 while moving from 1 to 9, and 5 was already used, ignore the second pass.\n6. Pivot Analysis: Clearly distinguish between 'passing near' a dot and 'pivoting' on a dot.\n\nDO NOT GUESS. DO NOT ASSUME SYMMETRY. SPEND MAXIMUM TIME ANALYZING THE ENTIRE DURATION OF THE DRAWING.\n\nReturn ONLY a raw JSON object:\n{\n  \"success\": true | false,\n  \"type\": \"PATTERN\" | \"PIN\" | \"PASSWORD\",\n  \"key\": [1, 5, 2, ...] | \"1234\",\n  \"confidence\": 0.0 to 1.0,\n  \"analysis_log\": \"Short summary of pivot points found\",\n  \"error\": \"Reason if success is false\"\n}" }
              ]
            }],
