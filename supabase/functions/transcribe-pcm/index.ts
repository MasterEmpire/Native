      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (isSetupComplete) {
            ...
            // Using compliant snake_case and media_chunks as expected by the official Gemini Live API schema
            const mediaPayload = {
              realtime_input: {
                media_chunks: [
                  {
            ...
            // Transmit text prompt via client_content and mark turn_complete: true
            const textPayload = {
              client_content: {
                turns: [
                  {
